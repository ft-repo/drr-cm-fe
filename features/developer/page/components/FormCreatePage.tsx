...
const FormCreatePage: React.FC<Props> = (props) => {
  const { } = props
  ...
  const onSubmit = useCallback(async (value: FieldType) => {
    ...
    try {
      const api = await fetch('/api/create/page', {
        method: 'POST',
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
            <Controller
              ...
              control={control}
              rules={{
                required: 'กรุณาระบุชื่อ Page หลัก'
              }}
              render={({ field }) => {
                return (
                  <fieldset>
                    <label>ชื่อ Page หลัก <span className='text-red-500'>*</span></label>
                    <Input
                      {...field}
                      name={field.name}
                      placeholder='กรุณาระบุชื่อ Page หลัก'
                      className='w-full'
                      size='large'
                      style={{
          ...
          {watch('has_sub') &&
            <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
              <Controller
                ...
                control={control}
                rules={{
                  required: 'กรุณาระบุชื่อ Page ย่อย'
                }}
                render={({ field }) => {
                  return (
                    <fieldset>
                      <label>ชื่อ Page ย่อย <span className='text-red-500'>*</span></label>
                      <Input
                        {...field}
                        name={field.name}
                        placeholder='กรุณาระบุชื่อ Page ย่อย'
                        className='w-full'
                        size='large'
                        style={{
                          fontFamily: 'Noto Sans Thai'
...
export default React.memo<Props>(FormCreatePage)
